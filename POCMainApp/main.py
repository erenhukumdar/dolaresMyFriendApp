...
class MyFriendApp (object):
    ...
    def locate_dialog_tag(self):
        memory = self.session.service('ALMemory')
        memory.raiseEvent('MyFriend/DayPart', self.par_of_the_day())
    ...
    def launch_app(self, value):
        ...
        try:
            ...
            choices = {'0': (self.empty_app_id, self.empty_app_id, 1), '1': (self.faq_app_id, self.faq_app_id, 1),
                       '2': (self.training_app_id, self.training_app_id, 1),
                       '3': (self.auth_launcher_id, self.qmatic_app_id, 0), '4': (self.game_app, self.game_app, 1),
                       '5': (self.selfie_app, self.selfie_app, 1), '6': (self.safe_app, self.safe_app, 1),
                       '7': (self.finie_app, self.finie_app, 1),
                       '8': (self.finie_app, self.finie_app, 1)}
